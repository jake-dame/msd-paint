...
class StudioViewModel : ViewModel() {
    ...
    fun sensorSetUp(context: Context) {
        ...
        if (accelerometer != null) {
            ...
            collectLinearAccelerator(accelMagFlow)
        } else {
            Log.e("StudioFragment", "Accelerometer is null")
        }

        val myFlow = getSensorData(sensor!!, sensorManager)
        collectAndApplySensor(myFlow)

    }

    private fun collectAndApplySensor(flow: Flow<FloatArray>) {
        ...
        viewModelScope.launch {
            ...
            flow.collect { sensorValues ->
                ...
                val constrainedX = (marbleOffset.value!!.x + xVelocity * scale).coerceIn(
                    _brushSize.value!!,
                    rightBounds
                )
                val constrainedY = (marbleOffset.value!!.y + yVelocity * scale).coerceIn(
                    _brushSize.value!!,
                    bottomBounds
                )
        ...
        return channelFlow {
            ...
            val listener = object : SensorEventListener {
                override fun onSensorChanged(event: SensorEvent?) {
                    event?.let { channel.trySend(event.values).isSuccess }
                }
    ...
    private fun collectLinearAccelerator(flow: Flow<Float>) {
